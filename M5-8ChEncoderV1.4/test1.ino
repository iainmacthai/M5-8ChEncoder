...
const char* ssid = "MacOppo";
const char* password = "flipflop5187";
const char* mqtt_server = "192.168.130.196";
const char* mqtt_user = "admin";    // Add if your broker requires
const char* mqtt_pass = "admin";    // Add if your broker requires

WiFiClient espClient;
PubSubClient client(espClient);
bool wifiConnected = false;
bool mqttConnected = false;
bool mqttFirstCheck = true; 

// Connection retry intervals
unsigned long wifiReconnectPrevious = 0;
unsigned long mqttReconnectPrevious = 0;
...
void reconnect() {
  ...
  String clientId = "M5Dial-";
  clientId += String(WiFi.macAddress());
  
  if (client.connect(clientId.c_str(), mqtt_user, mqtt_pass)) {Serial.println("MQTT connected!");
    mqttConnected = true;
    client.publish("encoder/status", "M5Dial Connected");
    lv_obj_set_style_text_color(ui_MQTT, mqttConnected ? lv_color_hex(0x00FF00) : lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);    
  } else {
...
// Generate JSON payload with ALL encoder values
// Generate JSON payload with ALL encoder values
void publishAllEncoders() {
    if (client.connected()) {
        // Increased buffer size to 512 for safety
        char payload[512];
        snprintf(payload, sizeof(payload), 
            ...
            "{\"i\":7,\"v\":%d,\"s\":\"%s\"},"
            "{\"i\":8,\"v\":%d,\"s\":\"%s\"}]}",
            current_val[0], locked[0] ? "L" : "A",
            current_val[1], locked[1] ? "L" : "A",
            current_val[2], locked[2] ? "L" : "A",
            current_val[3], locked[3] ? "L" : "A",
            current_val[4], locked[4] ? "L" : "A",
            current_val[5], locked[5] ? "L" : "A",
            current_val[6], locked[6] ? "L" : "A",
            current_val[7], locked[7] ? "L" : "A"
        );
        
        client.publish("Encoders-All", payload);
    }
}
...
void UpdateChannel(uint8_t channel, lv_obj_t* arc, lv_obj_t* valueLabel, lv_obj_t* btnLabel) {
    ...
    if (btn != last_btn[channel]) {
        if (btn) {
            ...
            sensor.setLEDColor(channel, locked[channel] ? 0xFF0000 : 0x00FF00);
            
            // Publish all encoders when any lock state changes
            publishAllEncoders();
        }
...
void loop() {
  ...
  bool currentWifi = (WiFi.status() == WL_CONNECTED);
    if (currentWifi != wifiConnected) {
        ...
        lv_obj_set_style_text_color(ui_WiFi, wifiConnected ? lv_color_hex(0x00FF00) : lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);
    }

  // Handle MQTT connection
  bool currentMqtt = client.connected(); // Get fresh connection state
    if (currentMqtt != mqttConnected || mqttFirstCheck) {
        mqttConnected = currentMqtt;
        Serial.println(mqttConnected ? "MQTT connected!" : "MQTT disconnected!");
        // Update MQTT label color
        lv_obj_set_style_text_color(ui_MQTT, mqttConnected ? lv_color_hex(0x00FF00) : lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);


        if (mqttFirstCheck) mqttFirstCheck = false;
    }

  // Reconnect logic
  if (!wifiConnected && millis() - wifiReconnectPrevious >= reconnectInterval) {
    wifiReconnectPrevious = millis();
    Serial.println("Reconnecting WiFi...");
    WiFi.disconnect();
    WiFi.begin(ssid, password);
  }

  if (wifiConnected && !mqttConnected && millis() - mqttReconnectPrevious >= reconnectInterval) {
    mqttReconnectPrevious = millis();
    reconnect();
  }
